...
(defalias (quote y-next) (function (lambda (h) (if (keywordp (car h)) (cddr h) (cdr h)))))
...
(defalias (quote y-get) (function (lambda (h k) (if (hash-table-p h) (gethash k h) (if (listp h) (catch (quote y-break) (let* ((i0 -1)) (while h (let* ((var (if (keywordp (car h)) (y-%key (car h)) (setq i0 (1+ i0)))) (val (if (keywordp (car h)) (cadr h) (car h)))) (if (eq var k) (progn (throw (quote y-break) val)))) (setq h (if (keywordp (car h)) (cddr h) (cdr h)))) nil)) (elt h k))))))
(quote byte-compile-inline-expand)
...
(defalias (quote y-wipe) (cons (quote macro) (function (lambda (place) (gv-get place (function (lambda (getter _setter) (if (eq (quote y-get) (car-safe getter)) (list (quote y-set) (cadr getter) (cons (quote y-put) (cdr getter))) (error (format "Can't wipe %S" place))))))))))
...
(defalias (quote y-setenv) (function (lambda (k &rest ks) (let* ((i (if (memq :toplevel ks) 0 (- (y-length y-environment) 1)))) (let* ((frame (y-get y-environment i))) (let* ((entry (or (y-get frame k) (make-hash-table :test (quote eq))))) (progn (let* ((o0 ks)) (let* ((f0 (function (lambda (k v) (let* ((k0 k) (v0 v)) (setq entry (y-put entry k0 v0)) v0))))) (progn (if (hash-table-p o0) (maphash f0 o0) (if (listp o0) (let* ((i14 -1)) (while o0 (let* ((k (if (keywordp (car o0)) (y-%key (car o0)) (setq i14 (1+ i14)))) (a0 (if (keywordp (car o0)) (cadr o0) (car o0)))) (funcall f0 k a0)) (setq o0 (if (keywordp (car o0)) (cddr o0) (cdr o0)))) nil) (let* ((n0 (y-length o0))) (progn (let* ((k 0)) (progn (while (< k n0) (let* ((a0 (y-get o0 k))) (progn (funcall f0 k a0))) (setq k (+ k 1)))))))))))) (let* ((k1 k) (v1 entry)) (setq frame (y-put frame k1 v1)) v1))))))))
...
(y-setenv (quote add) :macro (function (lambda (l x) (progn (or (y-atom-p l) (signal (quote cl-assertion-failed) (list (quote (y-atom-p l))))) nil) (cons (quote progn) (cons (list (quote set) (list (quote at) l (list (quote \#) l)) x) (quote (nil)))))))

(y-setenv (quote drop) :macro (function (lambda (l) (progn (or (y-atom-p l) (signal (quote cl-assertion-failed) (list (quote (y-atom-p l))))) nil) (list (quote prog1) (list (quote at) l (list (quote edge) l)) (list (quote set) l (list (quote almost) l))))))
...
(defalias (quote y-join) (function (lambda (&rest ls) (let* ((r (y-dup (y-hd ls)))) (progn (let* ((x0 ls)) (let* ((n3 (y-length x0))) (progn (let* ((i4 0)) (progn (while (< i4 n3) (let* ((l (y-get x0 i4))) (progn (if l (progn (let* ((n (y-length r))) (progn (let* ((o3 l)) (let* ((f3 (function (lambda (k v) (if (y-number-p k) (setq k (+ k n))) (let* ((k6 k) (v6 v)) (setq r (y-put r k6 v6)) v6))))) (progn (if (hash-table-p o3) (maphash f3 o3) (if (listp o3) (let* ((i17 -1)) (while o3 (let* ((k (if (keywordp (car o3)) (y-%key (car o3)) (setq i17 (1+ i17)))) (a3 (if (keywordp (car o3)) (cadr o3) (car o3)))) (funcall f3 k a3)) (setq o3 (if (keywordp (car o3)) (cddr o3) (cdr o3)))) nil) (let* ((n4 (y-length o3))) (progn (let* ((k 0)) (progn (while (< k n4) (let* ((a3 (y-get o3 k))) (progn (funcall f3 k a3))) (setq k (+ k 1)))))))))))))))))) (setq i4 (+ i4 1)))))))) r)))))
(y-setenv (quote join) :symbol (quote y-join))

(defalias (quote y-find) (function (lambda (f l) (catch (quote y-break) (let* ((o4 l)) (let* ((f4 (function (lambda (i5 x) (let* ((y (funcall f x))) (progn (if y (throw (quote y-break) y)))))))) (progn (if (hash-table-p o4) (maphash f4 o4) (if (listp o4) (let* ((i18 -1)) (while o4 (let* ((i5 (if (keywordp (car o4)) (y-%key (car o4)) (setq i18 (1+ i18)))) (a4 (if (keywordp (car o4)) (cadr o4) (car o4)))) (funcall f4 i5 a4)) (setq o4 (if (keywordp (car o4)) (cddr o4) (cdr o4)))) nil) (let* ((n5 (y-length o4))) (progn (let* ((i5 0)) (progn (while (< i5 n5) (let* ((a4 (y-get o4 i5))) (progn (funcall f4 i5 a4))) (setq i5 (+ i5 1))))))))))))))))
...
(defalias (quote y-map) (function (lambda (f x) (let* ((l (y-dup x))) (progn (let* ((x2 x)) (let* ((n7 (y-length x2))) (progn (let* ((i7 0)) (progn (while (< i7 n7) (let* ((v (y-get x2 i7))) (progn (let* ((y (funcall f v))) (progn (if (y-is-p y) (progn (let* ((k8 (y-length l)) (v8 y)) (setq l (y-put l k8 v8)) v8) nil)))))) (setq i7 (+ i7 1)))))))) (let* ((o5 x)) (let* ((f5 (function (lambda (k v) (if (y-number-p k) nil (let* ((y (funcall f v))) (progn (if (y-is-p y) (progn (let* ((k9 k) (v9 y)) (setq l (y-put l k9 v9)) v9)))))))))) (progn (if (hash-table-p o5) (maphash f5 o5) (if (listp o5) (let* ((i19 -1)) (while o5 (let* ((k (if (keywordp (car o5)) (y-%key (car o5)) (setq i19 (1+ i19)))) (a5 (if (keywordp (car o5)) (cadr o5) (car o5)))) (funcall f5 k a5)) (setq o5 (if (keywordp (car o5)) (cddr o5) (cdr o5)))) nil) (let* ((n8 (y-length o5))) (progn (let* ((k 0)) (progn (while (< k n8) (let* ((a5 (y-get o5 k))) (progn (funcall f5 k a5))) (setq k (+ k 1)))))))))))) l)))))
...
(defalias (quote y-keys-p) (function (lambda (l) (catch (quote y-break) (let* ((o6 l)) (let* ((f6 (function (lambda (k v) (if (y-number-p k) nil (throw (quote y-break) (quote t))))))) (progn (if (hash-table-p o6) (maphash f6 o6) (if (listp o6) (let* ((i20 -1)) (while o6 (let* ((k (if (keywordp (car o6)) (y-%key (car o6)) (setq i20 (1+ i20)))) (a6 (if (keywordp (car o6)) (cadr o6) (car o6)))) (funcall f6 k a6)) (setq o6 (if (keywordp (car o6)) (cddr o6) (cdr o6)))) nil) (let* ((n9 (y-length o6))) (progn (let* ((k 0)) (progn (while (< k n9) (let* ((a6 (y-get o6 k))) (progn (funcall f6 k a6))) (setq k (+ k 1)))))))))))) nil))))
(y-setenv (quote keys\?) :symbol (quote y-keys-p))

(defalias (quote y-empty-p) (function (lambda (l) (catch (quote y-break) (let* ((o7 l)) (let* ((f7 (function (lambda (i8 x) (throw (quote y-break) nil))))) (progn (if (hash-table-p o7) (maphash f7 o7) (if (listp o7) (let* ((i21 -1)) (while o7 (let* ((i8 (if (keywordp (car o7)) (y-%key (car o7)) (setq i21 (1+ i21)))) (a7 (if (keywordp (car o7)) (cadr o7) (car o7)))) (funcall f7 i8 a7)) (setq o7 (if (keywordp (car o7)) (cddr o7) (cdr o7)))) nil) (let* ((n10 (y-length o7))) (progn (let* ((i8 0)) (progn (while (< i8 n10) (let* ((a7 (y-get o7 i8))) (progn (funcall f7 i8 a7))) (setq i8 (+ i8 1)))))))))))) (quote t)))))
...
(defalias (quote y-write-file) (function (lambda (path data) (let ((temp-buffer (generate-new-buffer " *temp*"))) (save-current-buffer (set-buffer temp-buffer) (unwind-protect (progn (insert data) (write-region (point-min) (point-max) path nil t)) (and (buffer-name temp-buffer) (kill-buffer temp-buffer))))))))
